...
def compress_video(video_full_path, size_upper_bound, two_pass=True, filename_suffix='cps_'):
    ...
    try:
        ...
        print('Error: ', e)
        print('Install ffmpeg - enter following in terminal: brew install ffmpeg')
        return False

if __name__ == '__main__':
    target_size = input(print("Enter target size in KB: "))
    target_video = input(print("Enter video file name (must be in root directory): "))
    file_name = compress_video(target_video, int(target_size) * 1000)
    print(file_name)
